 ...
 * 然后需要完成新闻源的聚合，有一个Reading List，自己添加的内容也是其中一个源，左边栏显示源
 * 新闻源主要是方便抓取的新闻网站
 * 点选源后，在右侧主界面显示文章摘要
 * 点了阅读后，主界面显示文章内容，顶端显示一些信息和返回按钮
 * 【重点词】
 * 可以把文章中的词通过共现等原则提取出来比较重要的，然后做一个关联图展示
 * 目前的的罗列方式不利于查看，起码要分类
 * 目前把同一个单词的不同形式也都罗列了，这个需要去重
 * 也许可以用 word2vec 来评估单词的词形变化
 * 【重点句】
 * 需要用摘要算法找出在内容上关键的句子，可以试一试TextRank算法
 * 也需要用词权重找出来在用词上高级的句子，暂且先用现在的算法，再找找有没有更好的选择
 * 今天做词语之间的关联度分析可视化，也很有启发，如果用关联度矩阵或者相似矩阵，也许可以判断一个句子中是否运用了比较复杂和高级的语法结构。
 * 【翻译】
 * 还需要有全文翻译的功能或者分段翻译的功能
 * 调用有道接口实现逐句翻译，有道在长句时会炸，需要手动分词，并在重新排版中英对照后保留段落信息
 * 每一句的翻译是可以单独打开和关闭的，有时候我们只是想看看某一句的释义。
 * 【Word Wise】
 * 对于有难度的生词，直接将意思显示
 * 其他单词，点选后可以显示
